...
import { EnvCryptoService } from '../env/env.crypto';
import { LoginDto, RegisterDto } from './auth.dto';

@Injectable()
export class AuthService {
  constructor(private readonly prisma: PrismaService, private readonly jwt: JwtService,private readonly crypto:EnvCryptoService) {}
  ...
  private async createUser(input: RegisterDto, role: 'USER' | 'ADMIN') {
    ...
    const planId = await this.defaultPlanId();
    const user = await this.prisma.user.create({ data: { email, displayName: input.displayName.trim(), passwordHash: await hash(input.password, 12), role, planId } });
  ...
  async login(input: LoginDto) {
    ...
    await this.prisma.user.update({ where: { id: user.id }, data: { lastLoginAt: new Date(), planId: user.planId || await this.defaultPlanId() } });
    return this.session(user);
  }
  async discordEnabled(){const config=await this.discordConfig();return config.enabled&&Boolean(config.clientId&&config.clientSecret)}
  async discordAuthorizationUrl(){const config=await this.discordConfig();if(!config.enabled||!config.clientId||!config.clientSecret)throw new BadRequestException('Login com Discord não configurado');const state=await this.jwt.signAsync({purpose:'discord-oauth'},{expiresIn:'10m'});const url=new URL('https://discord.com/oauth2/authorize');url.searchParams.set('client_id',config.clientId);url.searchParams.set('response_type','code');url.searchParams.set('redirect_uri',config.redirectUri);url.searchParams.set('scope','identify email');url.searchParams.set('state',state);return{url:url.toString(),state}}
  async discordCallback(code:string,state:string,cookieState:string|undefined){if(!code||!state||!cookieState||state!==cookieState)throw new UnauthorizedException('Estado OAuth inválido');try{const payload=await this.jwt.verifyAsync(state);if(payload.purpose!=='discord-oauth')throw new Error()}catch{throw new UnauthorizedException('Estado OAuth expirado')};const config=await this.discordConfig();if(!config.enabled||!config.clientId||!config.clientSecret)throw new UnauthorizedException('Login com Discord desativado');const body=new URLSearchParams({client_id:config.clientId,client_secret:config.clientSecret,grant_type:'authorization_code',code,redirect_uri:config.redirectUri});const tokenResponse=await fetch('https://discord.com/api/oauth2/token',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body});if(!tokenResponse.ok)throw new UnauthorizedException('Discord recusou a autorização');const token:any=await tokenResponse.json();const profileResponse=await fetch('https://discord.com/api/users/@me',{headers:{authorization:`Bearer ${token.access_token}`}});if(!profileResponse.ok)throw new UnauthorizedException('Não foi possível obter o perfil do Discord');const profile:any=await profileResponse.json();if(!profile.id||!profile.email||profile.verified===false)throw new UnauthorizedException('O Discord precisa fornecer um e-mail verificado');const email=String(profile.email).toLowerCase(),discordAvatar=profile.avatar?`https://cdn.discordapp.com/avatars/${profile.id}/${profile.avatar}.png`:null,defaultPlanId=await this.defaultPlanId();let account=await this.prisma.oAuthAccount.findUnique({where:{provider_providerAccountId:{provider:'DISCORD',providerAccountId:String(profile.id)}},include:{user:true}});let user=account?.user;if(!user){user=await this.prisma.user.findUnique({where:{email}})||await this.prisma.user.create({data:{email,displayName:profile.global_name||profile.username||'Usuário Discord',avatarUrl:discordAvatar,emailVerifiedAt:new Date(),planId:defaultPlanId}});await this.prisma.oAuthAccount.create({data:{provider:'DISCORD',providerAccountId:String(profile.id),userId:user.id,expiresAt:token.expires_in?new Date(Date.now()+Number(token.expires_in)*1000):null}})}if(user.status!=='ACTIVE')throw new UnauthorizedException('Conta suspensa ou bloqueada');user=await this.prisma.user.update({where:{id:user.id},data:{lastLoginAt:new Date(),avatarUrl:user.avatarUrl||discordAvatar,planId:user.planId||defaultPlanId}});return this.session(user)}
  private async discordConfig(){const keys=['discord.enabled','discord.clientId','discord.clientSecret','discord.redirectUri'],rows=await this.prisma.appSetting.findMany({where:{key:{in:keys}}}),values=Object.fromEntries(rows.map(row=>[row.key,this.crypto.decrypt('platform-settings',row.key,{encryptedValue:row.encryptedValue,iv:row.iv,authTag:row.authTag,keyVersion:row.keyVersion})]));return{enabled:values['discord.enabled']?values['discord.enabled']==='true':Boolean(process.env.DISCORD_CLIENT_ID&&process.env.DISCORD_CLIENT_SECRET),clientId:values['discord.clientId']||process.env.DISCORD_CLIENT_ID||'',clientSecret:values['discord.clientSecret']||process.env.DISCORD_CLIENT_SECRET||'',redirectUri:values['discord.redirectUri']||process.env.DISCORD_REDIRECT_URI||`${(process.env.PUBLIC_URL||process.env.WEB_URL||'http://localhost:3000').replace(/\/$/,'')}/api/auth/discord/callback`}}
  private async defaultPlanId(){return (await this.prisma.resourcePlan.findFirst({where:{isDefault:true,enabled:true},select:{id:true}}))?.id||null}

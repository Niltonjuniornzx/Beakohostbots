...
import { Activity, Bot, Server, Settings, ShieldCheck, Users } from 'lucide-react';
...
export default function PageShell({ children, adminOnly=false }: { children: ReactNode; adminOnly?: boolean }) {
  ...
  return <main className="shell"><aside><div className="brand"><div className="logo"><Bot size={23}/></div><div><b>BeakoHost</b><small>Bot Cloud</small></div></div><nav>
    {item('/', <Activity/>, 'Visão geral')}{item('/bots', <Bot/>, 'Meus bots')}{item('/security', <ShieldCheck/>, 'Segurança')}{role==='ADMIN'&&<><span className="navSection">ADMINISTRAÇÃO</span>{item('/admin', <Settings/>, 'Resumo')}{item('/admin/users', <Users/>, 'Usuários')}{item('/admin/servers', <Server/>, 'Servidores')}{item('/admin/bots', <Bot/>, 'Distribuição')}</>}
